...
| [**artisticPaintingWithHttpInfo**](ArtisticApi.md#artisticPaintingWithHttpInfo) | **POST** /image/artistic/painting/{style} | Transform an image into an artistic painting automatically |
...
import com.cloudmersive.client.native.ArtisticApi;
...
## artisticPaintingWithHttpInfo

> ApiResponse<byte[]> artisticPainting artisticPaintingWithHttpInfo(style, imageFile)
...
public class Example {
    public static void main(String[] args) {
        ...
        File imageFile = new File("/path/to/file"); // File | Image file to perform the operation on.  Common file formats such as PNG, JPEG are supported.
        try {
            ApiResponse<byte[]> response = apiInstance.artisticPaintingWithHttpInfo(style, imageFile);
